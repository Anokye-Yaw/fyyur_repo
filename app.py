...
def search_venues():
  ...
  search_by_venue = request.form.get("search_by_venue", "")
  response = {}
  venues = list(Venue.query.filter(
        Venue.name.ilike(f"%{search_by_venue}%") |
        Venue.state.ilike(f"%{search_by_venue}%") |
        Venue.city.ilike(f"%{search_by_venue}%") 
    ).all())
  ...
  response["data"].append(each_venue)
  return render_template('pages/search_venues.html', results=response, search_by_venue=search_by_venue)
...
def show_venue(venue_id):
  ...
  # previous shows
    previous_shows = db.session.query(Show).filter(Show.venue_id == venue_id).filter(
    ...
    setattr(venue,"past_shows_count", len(previous_shows))
...
def show_artist(artist_id):
  ...
  # TODO: replace with real artist data from the artist table, using artist_id
    ...
    #past show
    past_shows_qeury = db.session.query(Show).join(Artist).filter(Show.artist_id==artist_id).filter(Show.start_time<datetime.now()).all()     
    past_shows = []
    
    for show in artist.past_shows_query:
    ...
    data = {
        ...
        "past_shows_count": len(past_shows_qeury),
        ...
        for fieldname in request.form:
            if fieldname == 'genres':
              #dropdown multiple selection
                setattr(artist, fieldname, request.form.getlist(fieldname))
            elif fieldname == 'seeking_talent':
              #checkbox selection
                setattr(artist, fieldname, True if request.form.get(
                    fieldname) in ('y', True, 't', 'True') else False)
            else:
              #textfield selection
                setattr(artist, fieldname, request.form.get(fieldname))
  ...
  # venue record with ID <venue_id> using the new attributes
    ...
    try:
        for fieldname in request.form:
            if fieldname == 'genres':
              #dropdown multiple selection
                setattr(venue, fieldname, request.form.getlist(fieldname))
            elif fieldname == 'seeking_talent':
              #checkbox selection
                setattr(venue, fieldname, True if request.form.get(
                    fieldname) in ('y', True, 't', 'True') else False)
            else:
              #textfield selection
                setattr(venue, fieldname, request.form.get(fieldname))
    ...
    return redirect(url_for('show_venue', venue_id=venue_id))              
     #**************************************************#
     #****  END EDIT VENUE SUBMISSION FUNCTION  ********#   
     #**************************************************#  
